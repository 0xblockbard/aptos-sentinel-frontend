...
function CreateNewCampaignFormSubmitButton() {
    ...
    return (
      <div>
        <button
          className="create_campaign flex m-auto px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500"
...
function UpdateCampaignFormSubmitButton() {
    ...
    return (
      <div>
        <button
          className="update_campaign flex m-auto px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500"
...
function ContributeButton() {
    ...
    return (
      <div>
        <button
          id="pledge"
          className={`pledge ${!account ? 'opacity-40 cursor-not-allowed' : ''} bg-sky-500 flex items-center justify-center text-white active:bg-sky-600 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150`}
        ...
        {campaigns?.map((campaign, index) => (
          <div key={index} className="flex flex-col rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="flex-shrink-0 relative">
              ...
              {/* Progress Bar */}
              <div className="relative -top-3 overflow-hidden h-3 text-xs flex bg-sky-200">
                <div
                  style={{
                    width: `${(campaign.contributed_amount / campaign.funding_goal) * 100}%`,
                  }}
                  className="progress_bar shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-sky-600"
                ></div>
              </div>
            </div>
